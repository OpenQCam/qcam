  ...
  const Boolean K8_CONNECTIVITY)
{
  ...
  //label(&row[0]) = new_label(row[0]);
    label.labelimg[row[0]-*label.piximg]=new_label(row[0]);
  ...
  for (int r = 1; r < height; ++r)    {
    ...
    for (int c = 1; c < width; ++c) {
      ...
      else label(&row[c]) = new_label(row[c]);

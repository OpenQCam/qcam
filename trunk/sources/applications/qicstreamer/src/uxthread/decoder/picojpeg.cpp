...
#include "uxthread/decoder/picojpeg.h"
...
unsigned char pjpeg_need_bytes_callback(unsigned char* pBuf, unsigned char buf_size, unsigned char *pBytes_actually_read, void *pCallback_data)
{
    ...
    memcpy(pBuf, g_pInBuf + g_nInFileOfs, n);
...
// subsampling factor).
uint8 *jpegdecode(uint8* pInBuf, uint nInFileSize, int *x, int *y, int *comps, int reduce)
{
...
static void convertCb(uint8 dstOfs)
{
    ...
    for (i = 64; i > 0; i--)
    {
        ...
        cbG = ((cb * 88U) >> 8U) - 44U;
        clampcbG = subAndClamp(pDstG[0], cbG);
        *pDstG++ = clampcbG;//subAndClamp(*pDstG, cbG);

        cbB = (cb + ((cb * 198U) >> 8U)) - 227U;
        clampcbB = addAndClamp(pDstB[0], cbB);
        *pDstB++ = clampcbB;//addAndClamp(*pDstB, cbB);
    }
}
/*----------------------------------------------------------------------------*/
...
static void convertCr(uint8 dstOfs)
{
    ...
    for (i = 64; i > 0; i--)
    {
        ...
        crR = (cr + ((cr * 103U) >> 8U)) - 179;
        clampcrR = addAndClamp(pDstR[0], crR);
        *pDstR++ = clampcrR;

        crG = ((cr * 183U) >> 8U) - 91;
        clampcrG = subAndClamp(pDstG[0], crG);
        *pDstG++ = clampcrG;
    }
}
/*----------------------------------------------------------------------------*/

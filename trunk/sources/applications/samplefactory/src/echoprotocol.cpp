...
bool EchoProtocol::SignalInputData(IOBuffer &buffer) {
	...
	if (GetFarProtocol()->GetType() == PT_INBOUND_HTTP) {
		...
		_outputBuffer.ReadFromString("ECHO Test HAHAHA");
    ...
    Variant reqvar = pHTTP->GetHeaders();
    //string host = (string) reqvar["headers"]["Host"];
    INFO("my test %s %s", STR(reqvar["headers"]["Host"]), STR(reqvar["firstLine"]["url"]));

...
int getbsbuf (bitstream_t *bs, char* bsbuf)
{
  int i;

  for (i =0; i<(bs->buf_size-bs->buf_byte_idx-1); i++) {
    bsbuf[i] = bs->buf[bs->buf_size-1-i]; 
  }
...
void open_bit_stream_w(bitstream_t* bs, int size)
{
/* NO LONGER DONE HERE

...
void L3_set_config_mpeg_defaults(mpeg_t *mpeg) {
  ...
  mpeg->type = 0;   // TYPE_MPEG_I:1; TYPE_MPEG_II:0, default is 1
  mpeg->layr = 2;   // LAYR_III
  mpeg->mode = 3;   // MODE_DUAL:2; MODE_MONO:3, default is 2 
  mpeg->bitr = 64;
  mpeg->psyc = 2;   // PSYC_ATT
  mpeg->emph = 0;   // EMPH_NONE
  mpeg->crc  = 0;
  mpeg->ext  = 0;
  mpeg->mode_ext  = 0;
  mpeg->copyright = 0;
  mpeg->original  = 0; // default is 1

}


int L3_find_samplerate_index(long freq, int mpegtype)
{
  static long mpeg1[3] = {44100, 48000, 32000};
  static long mpeg2LSF[3] = {22050, 24000, 16000};
  int i;
...
int L3_find_bitrate_index(int bitr, int mpegtype)
{
...
void L3_compress(config_t *config)
{
  ...
  config->mpeg.mode_gr           = (config->mpeg.type==1)?   2:  1;
  config->mpeg.samples_per_frame = (config->mpeg.type==1)?1152:576;  //config->mpeg.samples_per_frame = samp_per_frame;
  ...
  //while(config->get_pcm(buffer, config))
  {
    ...
    mean_bits = (config->mpeg.bits_per_frame - sideinfo_len) / config->mpeg.mode_gr; //>>1;

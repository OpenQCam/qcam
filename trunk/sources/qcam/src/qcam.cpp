...
bool Initialize() {
...
#else
    FATAL("Unable to support xml file");
#endif
  ...
  if(!HardwareManager::InitHardwareDevice(gRs.pConfigFile->GetHardwareConfiguration())){
    FATAL("Unable to initialize all hardware devices");
    return false;
  }
...
void Cleanup() {
  WARN("Shutting down Hardware");
  HardwareManager::ShutdownHardware();
  ...
  //TODO(Tim): need to implement shutdown procedure
  HardwareManager::ShutdownHardware();
...
#endif
#ifdef HAS_APP_QICSTREAMER
extern "C" BaseClientApplication *GetApplication_qicstreamer(Variant configuration);
#endif
...
#endif
#ifdef HAS_APP_QICSTREAMER
  else if (configuration[CONF_APPLICATION_NAME] == "qicstreamer") {
    return GetApplication_qicstreamer(configuration);
  }
#endif
  else {
    return NULL;

...
public:
  ...
  /*!
    @brief Find first stream by type and by name
    @param type - the complete/mask type to search for
    @param name - the complete/partial name to search for
  */
  BaseStream* FindFirstByTypeByName(uint64_t type, string name);

  /*!
    @brief Find first stream by type
    @param type - the complete/mask type to search for
   */
  BaseStream* FindFirstByType(uint64_t type);
   ...
   */
  const BaseClientApplication* GetApplication() {return _pApplication;}

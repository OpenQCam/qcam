...
private:
  ...
  //Tim: make action handler be seen by any protocol handlers
...
public:
  ...
  //Tim: support unix domain socket
  virtual bool AcceptUnixDomainSocketConnection(UnixDomainSocketAcceptor *pUXAcceptor);

  /**
   *  @brief Register action handler into this application
   * **/
  ...
  virtual void SignalUnLinkingStreams(BaseInStream *pInStream,
                                      BaseOutStream *pOutStream);
  virtual bool OnNotifyEvent(uint16_t type, uint16_t eventType,
  ...
  virtual bool OnServerRegisterAuthed(Variant& authData);

...
private:
  ...
  uint32_t _offset;
public:
	AtomSTSC(BaseMP4Document *pDocument, uint32_t type, uint64_t size, uint64_t start);
	AtomSTSC(BaseMP4Document *pDocument, uint32_t firstChunk, uint32_t sampleDescriptionIndex);
	virtual ~AtomSTSC();

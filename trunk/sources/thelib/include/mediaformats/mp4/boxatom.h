...
private:
public:
	BoxAtom(BaseMP4Document *pDocument, uint32_t type, uint64_t size, uint64_t start);
	virtual ~BoxAtom();
  ...
  virtual void SetChildAtom(BaseAtom *pChild);
  virtual void SerializeToBuffer(IOBuffer& data, uint32_t maxFrames);
  virtual void ListSize(string indent) {
    FOR_VECTOR (_subAtoms, i) {
      _subAtoms[i]->ListSize(string(indent+"----"));
    }
    DEBUG (" %s%s[%llu]", STR(indent), STR(GetTypeString()), _size);
  }
protected:

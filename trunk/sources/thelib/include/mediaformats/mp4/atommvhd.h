...
private:
  ...
  double _firstTS;
public:
	AtomMVHD(BaseMP4Document *pDocument, uint32_t type, uint64_t size, uint64_t start);
  AtomMVHD(BaseMP4Document *pDocument, uint64_t creationTime, uint32_t timeScale, uint32_t nextTrakId);
	virtual ~AtomMVHD();

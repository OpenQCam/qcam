...
public:
	AtomFTYP(BaseMP4Document *pDocument, uint32_t type, uint64_t size, uint64_t start);
  AtomFTYP(BaseMP4Document *pDocument, uint32_t majorBrand, uint32_t minorVersion, vector<uint32_t> brands);
	virtual ~AtomFTYP();
	virtual bool Read();

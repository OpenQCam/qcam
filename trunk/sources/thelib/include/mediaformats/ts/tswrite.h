...
struct buffered_TS_output
{
  ...
  // `got_pcr` values whenever we start a newone circular buffer entry,
 ...
 *
 * - `buf` is a pointer to the newone shared memory
...
#else // _WIN32
  cb = (circular_buffer_p)mmap(NULL,total_size,
 ...
 *
 * - `writer` is the newone buffered output context
 * - `circ_buf_size` is the number of buffer entries (plus one) we would
                                    ...
                                    const tswrite_pkt_hdr_type_t   hdr_type)
{
  int err, ii;
  circular_buffer_p     circular;
  buffered_TS_output_p  newone = (buffered_TS_output_p)calloc(1, SIZEOF_BUFFERED_TS_OUTPUT);
  if (newone == NULL)
  {
    print_err("### Unable to allocate buffered output\n");
    return 1;
  }
  reset_pcr_time(&newone->pcr_pace, 0);
  ...
  newone->buffer  = circular;
  newone->started = FALSE;
  newone->which   = (circular->pending + 1) % circular->size;
  newone->num_packets = 0;

  newone->rate = rate;
  newone->pcr_mode = pcr_mode;
  newone->prime_size = prime_size;
  newone->prime_speedup = prime_speedup;

  newone->pcr_scale = pcr_scale;

  newone->pcr_pace.prime_speed = prime_speedup;
  newone->pcr_pace.prime_req = (prime_speedup != PRIME_SPEED_NORMAL);
  ...
  for (ii = 0; ii < MAX_TS_PACKETS_IN_ITEM; ii++)
    newone->packet[ii].got_pcr = FALSE;
  ...
  if (found_pcr)
  {
    ...
    else
    {
      ...
      if (!had_second_pcr)  // i.e., *this* is the second PCR
      {
        ...
        // on this newone rate (which will hopefully smooth out better)
...
set_circ_times(const circular_buffer_p circ,
    ...
    uint64_t * const pnewone_pcr_base)
{
  ...
  if (pnewone_pcr_base != NULL)
  {
    *pnewone_pcr_base = (int64_t)pcr1 + (int64_t)offset * (int64_t)pcr_gap / (int64_t)gap_bytes;
  }
 ...
 * - `writer` is our buffered output context
 *
 * Returns newone idx that can be written or -1 if unchanged
 */
...
#if DISPLAY_BUFFER
  if (global_show_circular) print_circular_buffer((char *)"eof",circular);
#endif
  circular->eos = TRUE;
  return 0;
}
 ...
 * starting a newone buffer item that contains a single byte, set to
                        ...
                        int      *command_changed)
{
  ...
  case '\n':  // newoneline is needed to send commands to us
#if DEBUG_COMMANDS
    print_msg("[[newoneline/ignored]]\n");
#endif
                          ...
                          int          data_len)
{
  ...
  else
  {
    ...
    while (not_written)
    {
      ...
      // Only look for a newone command if the last is not still outstanding
...

/*
 * Wait for a newone command after 'p'ausing.
 *
 * - `tswriter` is the TS output context returned by `tswrite_open`
 *
 * Returns 0 if all went well, 1 if something went wrong.
...
extern int wait_for_command(TS_writer_p  tswriter)
{
  if (tswriter->command_socket == -1)
  {
    print_err("### Cannot wait for newone command when command input"
              " is not enabled\n");
    return 1;
  }
  else
                               ...
                               const circular_buffer_p  circular)
{
  ...
  if (global_show_circular)
  {
    newoneend = circular->pending;
    newonestart = circular->start;
    if (oldend != newoneend || oldstart != newonestart)
    {
      fprint_msg("get [%2d,%2d] became [%2d,%2d]",
                 oldend,oldstart,newoneend,newonestart);
      if (oldstart != newonestart)
        print_msg(" (!!)");
      if (newonestart == (newoneend + 1) % circular->size)
        print_msg(" ->empty");
      if ((newoneend + 2) % circular->size == newonestart)
        print_msg(" ->full");
      print_msg("\n");
    }
  }
...
//    fprint_msg("%s: Discontinuity[%d]: reset=%d, pkt_time=%u\n", __func__, circular->start, reset, this_packet_time);
    ...
    // We believe out timeline has gone askew - start a newone one
...
{
  TS_writer_p  newone = NULL;
  newone = (TS_writer_p)malloc(SIZEOF_TS_WRITER);
  if (newone == NULL)
  {
  ...
  newone->count = 0;
  newone->quiet = quiet;
  newone->server = FALSE;            // not being a server
  newone->command_socket = -1;       // not taking commands
  newone->command = COMMAND_PAUSE;   // start in pause
  newone->command_changed = FALSE;   // no newone command
  newone->atomic_command = FALSE;    // but any command is interruptable
  newone->drop_packets = 0;
  *tswriter = newone;
  return 0;
}

 ...
 * - `tswriter` is the newone context to use for writing TS output,
                        ...
                        TS_writer_p    *tswriter)
{
  ...
  case TS_W_STDOUT:
    ...
    newone->where.file = stdout;
    break;
  case TS_W_FILE:
    if (!quiet) fprint_msg("Writing to file %s\n",name);
    newone->where.file = fopen(name,"wb");
    if (newone->where.file == NULL)
    {
  ...
  case TS_W_TCP:
    if (!quiet) fprint_msg("Connecting to %s via TCP/IP on port %d\n",
                           name,port);
    newone->where.socket = connect_socket(name,port,TRUE, NULL);
    if (newone->where.socket == -1)
    {
  ...
  case TS_W_UDP:
    ...
    newone->where.socket = connect_socket(name,port,FALSE,multicast_if);
    if (newone->where.socket == -1)
    {
      fprint_err("### Unable to connect to %s\n",name);
      return 1;
                                   ...
                                   TS_writer_p  *tswriter)
{
  ...
  newone->where.socket = accept(server_socket,NULL,NULL);
  if (newone->where.socket == -1)
  {
#ifdef _WIN32
    err = WSAGetLastError();
 ...
 * by a newone command. However, some (the skip forwards and backwards commands)
...
extern void tswrite_help_tuning(void)
{
  fprint_msg(
    ...
    "likewise. When a newone PCR is found, the number of bytes and given\n"
                                ...
                                TS_context_p    context)
{
  ...
  while (ii < argc)
  {
    if (!strcmp("-nopcrs",argv[ii]))
    {
      ...
      argv[ii] = (char*)TSWRITE_PROCESSED;
    }
    else if (!strcmp("-bitrate",argv[ii]))
    {
      ...
      argv[ii] = argv[ii+1] = (char*)TSWRITE_PROCESSED;
      ii++;
    }
    ...
    else if (!strcmp("-perturb",argv[ii]))
    {
      ...
      argv[ii] = argv[ii+1] = argv[ii+2] = argv[ii+3] = (char*)TSWRITE_PROCESSED;
      ii+=3;
    }
#if DISPLAY_BUFFER

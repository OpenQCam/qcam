...
#define DEBUGPROGRAM_DTS 0
#define DEBUGPROGRAM_WRITE_PACKETS 0
 ...
 */
static int newone_TS_reader(TS_reader_p  *tsreader)
{
  TS_reader_p newone = (TS_reader_p)malloc(SIZEOF_TS_READER);
  if (newone == NULL)
  {
  ...
  memset(newone, '\0', SIZEOF_TS_READER);
...
{
  TS_reader_p newone;
  int err = newone_TS_reader(&newone);
  if (err) return 1;
                                    ...
                                    TS_reader_p *tsreader)
{
  ...
  newone->read_fn = read_fn;
  newone->seek_fn = seek_fn;
                                      ...
                                      pidint_list_p *prog_list)
{
  ...
  if (DEBUGPROGRAM) print_data(TRUE,"Data",data,data_len,1000);
                          ...
                          int           *data_used)
{
  ...
  if (*data == NULL)
  {
    ...
    // if (DEBUGPROGRAM) print_data(TRUE,"PMT",payload,payload_len,1000);
    packet_data = payload + pointer + 1;
    packet_data_len = payload_len - pointer - 1;
    if (DEBUGPROGRAM) print_data(TRUE,"Data",packet_data,packet_data_len,1000);

    section_length = ((packet_data[1] & 0xF) << 8) | packet_data[2];

#if 0 // XXX
      print_msg("===========================================\n");
      print_data(TRUE,"build_pmt_data(newone)",packet_data,packet_data_len,packet_data_len);
#endif
 ...
 * - `pmt` is the newone PMT datastructure
 *
 * Returns 0 if all went well, 1 if something went wrong.
 */
                       ...
                       pmt_p         *pmt)
{
  ...
  if (verbose && program_info_length > 0)
  {
    ...
    print_descriptors(TRUE,(char*)"    ",NULL,&data[12],program_info_length);
  }
  ...
  *pmt = build_pmt(program_number,version_number,pcr_pid);
  ...
  while (stream_data_len > 0)
  {
    ...
    if (verbose)
    {
      ...
      if (ES_info_length > 0)
        print_descriptors(TRUE,(char*)"        ",NULL,&stream_data[5],ES_info_length);
    }
...
int SARRAYSIZE=40;
      char buf[SARRAYSIZE];
      snprintf(buf,SARRAYSIZE,"(%s)",(char*)h222_stream_type_str(stream_type));
 ...
 *   giving up)
 * - `pmt` is a newone datastructure representing the PMT found
 *
 ...
 * - `num_read` is the number of packets read to find the PMT (or before
 *   giving up)
 * - `pmt` is a newone datastructure containing the information from the PMT.
 *

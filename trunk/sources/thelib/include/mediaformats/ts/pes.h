...
// ============================================================
/*
 * Build a newone PES packet datastructure
 *
 * Returns 0 if all goes well, 1 if something goes wrong
 */
static int build_PES_packet_data(PES_packet_data_p *data)
{
  PES_packet_data_p newone = (PES_packet_data_p)malloc(SIZEOF_PES_PACKET_DATA);
  if (newone == NULL)
  {
  ...
  newone->data = NULL;
  newone->data_len = 0;
  newone->es_data_len = 0;
  newone->length = 0;
  newone->posn = 0;
  newone->is_video = TRUE;     // a guess
  newone->data_alignment_indicator = FALSE; // another
  newone->has_PTS = FALSE;     // assumed until told otherwise
 ...
 *
 * - `data` is the "newone" dummy PES packet. Do not free this,
...
// ============================================================
/*
 * Initialise a newone PID/PES data datastructure.
 */
static int init_peslist(peslist_p  list)
{
  ...
  list->data = (PES_packet_data**)malloc(SIZEOF_PES_PACKET_DATA*PESLIST_START_SIZE);
  if (list->data == NULL)
  {
...

/*
 * Build a newone PID/PES data datastructure.
 *
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int build_peslist(peslist_p  *list)
{
  peslist_p  newone = (peslist_p)malloc(SIZEOF_PESLIST);
  if (newone == NULL)
  {
...

/*
 * Start a newone PES packet for a PID.
 *
 * Creates a newone entry for the given PID, and returns the corresponding newone
 * PES packet.
 *
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
                                   ...
                                   PES_packet_data_p *data)
{
  ...
  if (err)
  {
    print_err("### Unable to build newone PES packet datastructure"
  ...
  // Otherwise, we need to add a newone entry to the list
  if (list->length == list->size)
  {
    int newonesize = list->size + PESLIST_INCREMENT;
    list->data = (PES_packet_data**)realloc(list->data,newonesize*SIZEOF_PES_PACKET_DATA);
    if (list->data == NULL)
    {
    ...
    list->pid = (uint32_t*)realloc(list->pid,newonesize*sizeof(uint32_t));
    if (list->pid == NULL)
    {
      print_err("### Unable to extend PID/PES data array\n");
      free_PES_packet_data(data);
                                  ...
                                  pmt_p          pmt)
{
  ...
  if (reader->give_warning)
  {
    ...
    report_pmt(FALSE,(char*)"      ",reader->program_map);
    print_err("    newone program information is:\n");
  ...
  // And use this newone information to determine/check our video/audio PIDs
  decide_pids(reader);
  return 0;
}

 ...
 *
 * - `reader` is the PES reader context corresponding to the newonely
 *   opened file.
 *
...
/*
 * Start a newone PES packet, for a given PID/stream id
 *
 ...
 */
static int start_newone_PES_packet(PES_reader_p       reader,
                                uint32_t           pid,
                                byte               payload[],
                                int                payload_len,
                                PES_packet_data_p *finished)
{
  ...
  fprint_msg(": start newone %s PES packet, payload_len = %d\n",
  ...
  // Anyway, start a newone PES packet for this TS packet's data
#if DEBUG_PES_ASSEMBLY
  ...
  if (err)
  {
    fprint_err("### Error trying to start a newone PES packet,"
                          ...
                          uint64_t *value)
{
  ...
  {
  case 2:  what = (char*)"PTS"; break;  // standalone
  case 3:  what = (char*)"PTS"; break;  // before a DTS
  case 1:  what = (char*)"DTS"; break;  // always after a PTS
  default: what = (char*)"PTS/DTS"; break;  // surely some mistake?
  }
                                        ...
                                        PES_packet_data_p *packet_data)
{
  ...
  for (;;)
  {
    ...
    else if (pid == reader->pmt_pid)
    {
      ...
      {
        // This is the start of a newone PMT packet, but we'd already
      ...
      if (err)
      {
        fprint_err("### Error %s PMT at " OFFSET_T_FORMAT "\n",
                   (payload_unit_start_indicator?"starting newone":"continuing"),
                   reader->posn);
              ...
              (pid == reader->audio_pid && !reader->video_only)))
    {
      ...
      if (payload_unit_start_indicator)
        err = start_newone_PES_packet(reader,pid,payload,payload_len,
                                   &finished);
      else
...
{
  int  err;
  PES_reader_p newone = (PES_reader_p)malloc(SIZEOF_PES_READER);
  if (newone == NULL)
  {
  ...
  newone->tsreader = NULL;  // for the moment, at least
  newone->psreader = NULL;  // for the moment, at least
  newone->is_TS = FALSE;    // for want of better
  newone->give_info = give_info;
  newone->give_warning = give_warnings;
  newone->posn = 0;
  newone->is_h264 = FALSE;
  newone->video_type = VIDEO_UNKNOWN;
  newone->packet = NULL;

  newone->program_number = 0;
  newone->program_map = NULL;
  newone->video_only = FALSE;
  newone->audio_stream_id = 0;

  newone->pmt_data = NULL;
  newone->pmt_data_len = 0;
  newone->pmt_data_used = 0;

  newone->video_pid = newone->audio_pid = 0;
  newone->pcr_pid = newone->pmt_pid = 0;
  newone->got_program_data = FALSE;

  newone->output_program_number = 0;
  newone->output_video_pid = newone->output_audio_pid = 0;
  newone->output_pcr_pid = newone->output_pmt_pid = 0;
  newone->override_program_data = FALSE;

  newone->output_dolby_stream_type =
    newone->dolby_stream_type = DVB_DOLBY_AUDIO_STREAM_TYPE;
  newone->override_dolby_stream_type = FALSE;

  newone->tswriter = NULL;
  newone->write_PES_packets = FALSE;
  newone->write_TS_packets = FALSE;
  newone->suppress_writing = TRUE;
  newone->dont_write_current_packet = FALSE;
  newone->pes_padding = 0;

  newone->debug_read_packets = FALSE;

  err = build_peslist(&newone->packets);
  if (err)
  {
  ...
  newone->deferred = NULL;
  newone->had_eof = FALSE;
  *reader = newone;
  return 0;
}
...
extern int read_next_PES_packet(PES_reader_p  reader)
{
  ...
  if (reader->debug_read_packets)
  {
    if (err==EOF)
      ...
      fprint_msg("<<newone   PES packet at " OFFSET_T_FORMAT ">>\n",
                 reader->packet->posn);
  }
#endif
...
extern int read_next_PES_ES_packet(PES_reader_p       reader)
{
  for (;;)
  {
    ...
    {
      if (reader->debug_read_packets)
        report_PES_data_array((char*)"",reader->packet->data,reader->packet->data_len,
                              TRUE);
...
#define MAX_PTS_VALUE 0x1FFFFFFFFLL
  ...
  if (value > MAX_PTS_VALUE)
  {
    ...
    switch (guard_bits)
    {
    case 2:  what = (char*)"PTS alone"; break;
    case 3:  what = (char*)"PTS before DTS"; break;
    case 1:  what = (char*)"DTS after PTS"; break;
    default: what = (char*)"PTS/DTS/???"; break;
    }

...
// ============================================================================
/*
 * Initialise a newone pid/int list datastructure.
 */
...

/*
 * Build a newone pid/int list datastructure.
 *
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
extern int build_pidint_list(pidint_list_p  *list)
{
  pidint_list_p  newone = (pidint_list_p)malloc(SIZEOF_PIDINT_LIST);
  if (newone == NULL)
  {
                                 ...
                                 int            program)
{
  ...
  {
    int newonesize = list->size + PIDINT_LIST_INCREMENT;
    list->number = (int*)realloc(list->number,newonesize*sizeof(int));
    if (list->number == NULL)
    {
...

/*
 * Build a newone PMT datastructure.
 *
 ...
 *
 * Returns (a pointer to) the newone PMT datastructure, or NULL if some error
 * occurs.
 */
                       ...
                       uint32_t PCR_pid)
{
  ...
  newone = (pmt_p)malloc(SIZEOF_PMT);
  if (newone == NULL)
  {
  ...
  newone->program_number = program_number;
  newone->version_number = version_number;
  newone->PCR_pid = PCR_pid;
  newone->program_info_length = 0;
  newone->program_info = NULL;

  if (init_pmt_streams(newone))
  {
    free(newone);
    return NULL;
  }
                             ...
                             byte      *ES_info)
{
  ...
  {
    int newonesize = pmt->streams_size + PMT_STREAMS_INCREMENT;
    pmt->streams = (pmt_stream_p)realloc(pmt->streams,newonesize*SIZEOF_PMT_STREAM);
    if (pmt->streams == NULL)
    {
      print_err("### Unable to extend PMT streams array\n");
      return 1;
    }
    pmt->streams_size = newonesize;
  }
                       ...
                       pmt_p   pmt)
{
  ...
  if (pmt->program_info_length > 0)
  {
    ...
    print_descriptors(is_msg,prefix,(char *)"   ",pmt->program_info,
    ...
    for (ii=0; ii<pmt->num_streams; ii++)
    {
      ...
      if (pmt->streams[ii].ES_info_length > 0)
      {
        ...
        print_data(is_msg,"      ES info",
                   ...
                   pmt->streams[ii].ES_info_length);
        print_descriptors(is_msg,prefix,(char *)"      ",
                          pmt->streams[ii].ES_info,
                          pmt->streams[ii].ES_info_length);
      }
    }

...
/*
 * Build a newone access unit datastructure.
 *
                                    ...
                                    uint32_t        index)
{
  ...
  access_unit_p  newone = (access_unit_p)malloc(SIZEOF_ACCESS_UNIT);
  if (newone == NULL)
  {
  ...
  err = build_nal_unit_list(&(newone->nal_units));
  if (err)
  {
    free(newone);
    *acc_unit = NULL;
  ...
  newone->index = index;
  newone->started_primary_picture = FALSE;
  newone->primary_start = NULL;
  newone->ignored_broken_NAL_units = 0;

  newone->frame_num = newone->field_pic_flag = newone->bottom_field_flag = 0;
                              ...
                              nal_unit_list_p  pending)
{
  ...
  {
    // Our caller should have started a newone access unit instead
...

/*
 * Build a newone access unit context datastructure.
 *
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
...
{
  int err;
  access_unit_context_p  newone = (access_unit_context_p)malloc(SIZEOF_ACCESS_UNIT_CONTEXT);
  if (newone == NULL)
  {
  ...
  newone->pending_nal = NULL;
  newone->end_of_stream = NULL;
  newone->end_of_sequence = NULL;
  newone->access_unit_index = 0;
  newone->reverse_data = NULL;
  newone->no_more_data = FALSE;
  newone->earlier_primary_start = NULL;

  err = build_nal_unit_context(es,&newone->nac);
  if (err)
  {
  ...
  err = build_nal_unit_list(&newone->pending_list);
  if (err)
  {
    print_err("### Error building access unit context datastructure\n");
    free_nal_unit_context(&newone->nac);
    free(newone);
    return err;
  }
...

/*
 * Free a newone access unit context datastructure.
 *
                                ...
                                access_unit_p        *ret_access_unit)
{
  ...
  // Since we're expecting to return a newone access unit,
  ...
  for (;;)
  {
    ...
    /*if (nal_is_slice(nal))
    {
      if (!access_unit->started_primary_picture)
      {
        // We're in a newone access unit, but we haven't had a slice
        ...
        // a good idea)
        char start_reason[64]="First slice of newone access unit";
        nal->start_reason = start_reason;
      ...
      else if (nal_is_first_VCL_NAL(nal,context->earlier_primary_start))
      {
        ...
        if (access_unit->started_primary_picture)
        {
          ...
          // picture, so this NAL unit must start a newone access unit.
      ...
      {
        // But this is not a NAL unit that may start a newone
    ...
    else if (nal->nal_unit_type == NAL_ACCESS_UNIT_DELIM)
    {
      ...
      else
      {
            ...
            access_unit->nal_units->length > 0)
        {
          ...
          {
            char str[8]="    ";
            report_nal_unit_list(FALSE,str,access_unit->nal_units);
            reset_nal_unit_list(access_unit->nal_units,TRUE);
          }
          if (context->pending_list->length > 0)
          {
            char str[8]="    ";
            report_nal_unit_list(FALSE,str,context->pending_list);
    ...
    {
      // These start a newone access unit *if* they come after the
                               ...
                               access_unit_p        *frame)
{
  ...
  //*frame = NULL;
//FATAL("get_next_non_empty_access_unit");

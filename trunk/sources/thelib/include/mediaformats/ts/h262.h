...
extern void print_h262_start_code_str(byte   start_code)
{
  ...
  {
    // H.262 start codes
  case 0x00: str = (char*)"Picture"; break;
  case 0xB0: str = (char*)"Reserved"; break;
  case 0xB1: str = (char*)"Reserved"; break;
  case 0xB2: str = (char*)"User data"; break;
  case 0xB3: str = (char*)"SEQUENCE HEADER"; break;
  case 0xB4: str = (char*)"Sequence error"; break;
  case 0xB5: str = (char*)"Extension start"; break;
  case 0xB6: str = (char*)"Reserved"; break;
  case 0xB7: str = (char*)"SEQUENCE END"; break;
  case 0xB8: str = (char*)"Group start"; break;
  ...
  case 0xBC: str = (char*)"SYSTEM START: Program stream map"; break;
  case 0xBD: str = (char*)"SYSTEM START: Private stream 1"; break;
  case 0xBE: str = (char*)"SYSTEM START: Padding stream"; break;
  case 0xBF: str = (char*)"SYSTEM START: Private stream 2"; break;
  case 0xF0: str = (char*)"SYSTEM START: ECM stream"; break;
  case 0xF1: str = (char*)"SYSTEM START: EMM stream"; break;
  case 0xF2: str = (char*)"SYSTEM START: DSMCC stream"; break;
  case 0xF3: str = (char*)"SYSTEM START: 13522 stream"; break;
  case 0xF4: str = (char*)"SYSTEM START: H.222 A stream"; break;
  case 0xF5: str = (char*)"SYSTEM START: H.222 B stream"; break;
  case 0xF6: str = (char*)"SYSTEM START: H.222 C stream"; break;
  case 0xF7: str = (char*)"SYSTEM START: H.222 D stream"; break;
  case 0xF8: str = (char*)"SYSTEM START: H.222 E stream"; break;
  case 0xF9: str = (char*)"SYSTEM START: Ancillary stream"; break;
  case 0xFF: str = (char*)"SYSTEM START: Program stream directory"; break;
...

/*
 * Build a newone MPEG2 item datastructure.
 *
...
{
  int     err;
  h262_item_p  newone = (h262_item_p)malloc(SIZEOF_H262_ITEM);
  if (newone == NULL)
  {
...

/*
 * Build a newone H.262 picture reading context.
 *
...
{
  h262_context_p  newone = (h262_context_p)malloc(SIZEOF_H262_CONTEXT);
  if (newone == NULL)
  {
  ...
  newone->es = es;
  newone->picture_index = 0;
  newone->last_item = NULL;
  newone->reverse_data = NULL;
  newone->count_since_seq_hdr = 0;
  newone->last_aspect_ratio_info = H262_UNSET_ASPECT_RATIO_INFO;
  newone->last_afd = UNSET_AFD_BYTE;
  newone->add_fake_afd = FALSE;
...
/*
 * Build a newone H.262 "picture", starting with the given item (which is
                              ...
                              h262_item_p      item)
{
  ...
  ES_unit_p       unit = &(item->unit);
  byte           *data = unit->data;
  h262_picture_p  newone  = (h262_picture_p)malloc(SIZEOF_H262_PICTURE);
  if (newone == NULL)
  {
  ...
  err = build_ES_unit_list(&(newone->list));
  if (err)
  {
  ...
  {
    newone->picture_coding_type = item->picture_coding_type;
    newone->is_picture = TRUE;
    newone->is_sequence_header = FALSE;
    newone->temporal_reference = (data[4] << 2) | ((data[5] & 0xC0) >> 6);
    // Assume that our picture is a frame, until we're told otherwise
    // (MPEG-1 data will never tell us otherwise)
    newone->picture_structure = 3;
    newone->was_two_fields = FALSE;
    // Assume the last AFD and aspect ratio info until told otherwise
    newone->afd = context->last_afd;
    newone->aspect_ratio_info = context->last_aspect_ratio_info;
    newone->is_real_afd = FALSE;
  }
  else if (is_h262_seq_header_item(item))
  {
    newone->is_picture = FALSE;
    newone->is_sequence_header = TRUE;
    newone->picture_coding_type = 0; // Forbidden value, just in case
    newone->aspect_ratio_info = (data[7] & 0xF0) >> 4;
    // Assume that we are only allowed progressive frames, until we're told
    // otherwise (MPEG-1 data will never tell us otherwise)
    newone->progressive_sequence = 1;
  }
 ...
 */
static int get_next_field_of_pair_h262(h262_context_p  context,
                                  int             verbose,
                                  int             quiet,
                                  int             first_time,
                                  h262_picture_p *picture)
{
  ...
  else if (first_time)
  {
    ...
    free_h262_picture(picture);
    *picture = second;
    err = get_next_field_of_pair_h262(context,verbose,quiet,FALSE,picture);
    if (err) return 1;
  }
  else
  {
                               ...
                               h262_picture_p *picture)
{
  ...
  if (is_h262_field_picture(*picture))
  {
    ...
    err = get_next_field_of_pair_h262(context,verbose,quiet,TRUE,picture);
    if (err)
    {
      free_h262_picture(picture);
      return 1;
                                ...
                                int             report_data)
{
  ...
  if (report_data)
    report_ES_unit_list((char*)"ES units",picture->list);
}

// Local Variables:
// tab-width: 8

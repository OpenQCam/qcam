...
// ------------------------------------------------------------
/*
 * Create a newone "dictionary" for remembering picture or sequence
 * parameter sets.
 *
...
{
  param_dict_p  newone = (param_dict_p)malloc(SIZEOF_PARAM_DICT);
  if (newone == NULL)
  {
  ...
  newone->ids = (int*)malloc(sizeof(uint32_t)*NAL_PIC_PARAM_START_SIZE);
  if (newone->ids == NULL)
  {
  ...
  newone->params = (nal_innards*)malloc(SIZEOF_NAL_INNARDS*NAL_PIC_PARAM_START_SIZE);
  if (newone->params == NULL)
  {
    print_err("### Unable to allocate array within 'dictionary'"
              " datastructure\n");
    free(newone->ids);
  ...
  newone->posns = (ES_offset*)malloc(SIZEOF_ES_OFFSET*NAL_PIC_PARAM_START_SIZE);
  if (newone->posns == NULL)
  {
    print_err("### Unable to allocate array within 'dictionary'"
              " datastructure\n");
    free(newone->params);
  ...
  newone->data_lens = (uint32_t*)malloc(sizeof(uint32_t)*NAL_PIC_PARAM_START_SIZE);
  if (newone->data_lens == NULL)
  {
    print_err("### Unable to allocate array within 'dictionary'"
              " datastructure\n");
    free(newone->params);
  ...
  newone->size = NAL_PIC_PARAM_START_SIZE;
  newone->length = 0;
...
// ------------------------------------------------------------
/*
 * Build a newone NAL unit context, for reading NAL units from an ES.
 *
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
...
{
  int err;
  nal_unit_context_p  newone = (nal_unit_context_p)malloc(SIZEOF_NAL_UNIT_CONTEXT);
  if (newone == NULL)
  {
  ...
  newone->es = es;
  newone->count = 0;
  newone->show_nal_details = FALSE;
  err = build_param_dict(&newone->seq_param_dict);
  if (err)
  {
    free(newone);
    return err;
  }
  err = build_param_dict(&newone->pic_param_dict);
  if (err)
  {
    free_param_dict(&newone->seq_param_dict);
    free(newone);
    return err;
  }
  *context = newone;
...
// ------------------------------------------------------------
/*
 * Build a newone NAL unit datastructure.
 *
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
...
{
  int  err;
  nal_unit_p  newone = (nal_unit_p)malloc(SIZEOF_NAL_UNIT);
  if (newone == NULL)
  {
  ...
  newone->data = NULL;  // Only set to unit.data[3] when we *have* a NAL unit
  newone->data_len = 0;
  newone->rbsp = NULL;
  newone->rbsp_len = 0;
  newone->bit_data = NULL;

  newone->nal_unit_type = NAL_UNSPECIFIED;

  newone->starts_picture_decided = FALSE;
  newone->starts_picture = FALSE;
  newone->start_reason = NULL;
  newone->decoded = FALSE;
...
{
  uint32_t ii;
  for (ii=0; ii<(uint32_t)param_dict->length; ii++)
  {
    if ((uint32_t)param_dict->ids[ii] == param_id)
    {
...
#define CHECK(name) \
  ...
  // further. Assume the caller knewone what they were doing...
...

/*
 * Is this VCL NAL unit the first of a newone primary coded picture?
 *
 ...
 *
 *   The first NAL unit of a newone primary code picture can be detected
 *   because:
 *
                                ...
                                nal_unit_p   last)
{
  ...
  if (last == NULL)
  {
    ...
    // "start" a picture
    char str[64]="First slice in data stream";
    nal->start_reason = str;
    return TRUE;
  }
  ...
  if (thisS->frame_num != that->frame_num) {
    char str[64]="Frame number differs";
    nal->start_reason = str;
  }else if (thisS->field_pic_flag != that->field_pic_flag) {
    char str[64]="One is field, the other frame";
    nal->start_reason = str;
  }else if (thisS->bottom_field_flag_present &&
           that->bottom_field_flag_present &&
           thisS->bottom_field_flag != that->bottom_field_flag)
  {
    char str[64]="One is bottom field, the other top";
    nal->start_reason = str;
    ...
    char str[64]="One is reference picture, the other is not";
    nal->start_reason = str;
  }else if (thisS->seq_param_set_pic_order_cnt_type == 0 &&
           that->seq_param_set_pic_order_cnt_type == 0 &&
           (thisS->pic_order_cnt_lsb != that->pic_order_cnt_lsb ||
            thisS->delta_pic_order_cnt_bottom !=
            that->delta_pic_order_cnt_bottom)) {
    char str[64]="Picture order counts differ";
    nal->start_reason = str;
  }else if (thisS->seq_param_set_pic_order_cnt_type == 1 &&
           that->seq_param_set_pic_order_cnt_type == 1 &&
           (thisS->delta_pic_order_cnt[0] != that->delta_pic_order_cnt[0] ||
            thisS->delta_pic_order_cnt[1] != that->delta_pic_order_cnt[1])) {
    char str[64]="Picture delta counts differ";
    nal->start_reason = str;
  }else if ((nal->nal_unit_type == 5 || last->nal_unit_type == 5) &&
           nal->nal_unit_type != last->nal_unit_type) {
    char str[64]="One IDR, one not";
    nal->start_reason = str;
  }else if (nal->nal_unit_type == 5 && last->nal_unit_type == 5 &&
           thisS->idr_pic_id != that->idr_pic_id) {
    char str[64]="Different IDRs";
    nal->start_reason = str;
  }else
    nal->starts_picture = FALSE;
                          ...
                          nal_unit_p  nal)
{
  ...
  //forbidden_zero_bit = (nal->data[0] == 0x80)?1:0;
...
{
  int ii;
  if (param_id == (uint32_t)param_dict->last_id)
  {
  ...
  {
    int newonesize = param_dict->size + NAL_PIC_PARAM_INCREMENT;
    param_dict->ids = (int*)realloc(param_dict->ids,newonesize*sizeof(uint32_t));
    if (param_dict->ids == NULL)
    {
      print_err("### Unable to extend parameter set dictionary array\n");
      return 1;
    }
    param_dict->params = (nal_innards*)realloc(param_dict->params,
                                 newonesize*SIZEOF_NAL_INNARDS);
    if (param_dict->params == NULL)
    {
      print_err("### Unable to extend parameter set dictionary array\n");
      return 1;
    }
    param_dict->posns = (ES_offset*)realloc(param_dict->params,newonesize*SIZEOF_ES_OFFSET);
    if (param_dict->posns == NULL)
    {
      print_err("### Unable to extend parameter set dictionary array\n");
      return 1;
    }
    param_dict->data_lens = (uint32_t*)realloc(param_dict->params,
                                    newonesize*sizeof(uint32_t));
    if (param_dict->data_lens == NULL)
    {
      print_err("### Unable to extend parameter set dictionary array\n");
      return 1;
    }
    param_dict->size = newonesize;
  }
...
/*
 * Build a newone list-of-nal-units datastructure.
 *
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
extern int build_nal_unit_list(nal_unit_list_p  *list)
{
  nal_unit_list_p  newone = (nal_unit_list_p)malloc(SIZEOF_NAL_UNIT_LIST);
  if (newone == NULL)
  {
  ...
  newone->length = 0;
  newone->size = NAL_UNIT_LIST_START_SIZE;
  newone->array = (nal_unit**)malloc(sizeof(nal_unit_p)*NAL_UNIT_LIST_START_SIZE);
  if (newone->array == NULL)
  {
    free(newone);
                                   ...
                                   nal_unit_p       nal)
{
  ...
  {
    int newonesize = list->size + NAL_UNIT_LIST_INCREMENT;
    list->array = (nal_unit**)realloc(list->array,newonesize*sizeof(nal_unit_p));
    if (list->array == NULL)
    {
      print_err("### Unable to extend NAL unit list array\n");
      return 1;
    }
    list->size = newonesize;
  }

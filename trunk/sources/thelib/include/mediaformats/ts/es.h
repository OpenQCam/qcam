...
static inline int get_next_pes_packet(ES_p  es)
{
  ...
  // Point to our (newone) data buffer
...
static inline int get_more_data(ES_p  es)
{
  if (es->reading_ES)
  {
    ...
    int len=0;
    /*int jj;
    for (jj=0;jj<ES_READ_AHEAD_SIZE;jj++) {
      if(es->input_mm_size>=0) {
        es->read_ahead[jj]=es->input_mm[(es->input_bytes_had_read)+jj];
        es->input_mm_size--;
        len=jj+1;
      }
    }*/
    if(es->input_mm_size>ES_READ_AHEAD_SIZE) {
      memcpy(es->read_ahead, es->input_mm+es->input_bytes_had_read, ES_READ_AHEAD_SIZE);
      len=ES_READ_AHEAD_SIZE;
      es->input_mm_size-=ES_READ_AHEAD_SIZE;
    }else if(es->input_mm_size>0){
      memcpy(es->read_ahead, es->input_mm+es->input_bytes_had_read, es->input_mm_size);
      len=es->input_mm_size;
      es->input_mm_size=0;
    }

    if(len>0) {
      es->input_bytes_had_read+=len;
    }
    if (len == 0)
      return EOF;
    else if (len == -1)
...
extern int build_elementary_stream_file(int    input,
                                        ES_p  *es)
{
  ES_p newone = (ES_p)malloc(SIZEOF_ES);
  if (newone == NULL)
  {
  ...
  newone->reading_ES = TRUE;
  newone->input = input;
  newone->reader = NULL;

  setup_readahead(newone);

  *es = newone;
  return 0;
}

extern int build_elementary_stream_mm(uint8_t *input,
                                      int32_t  intput_size,
                                        ES_p  *es)
{
  ES_p newone = (ES_p)malloc(SIZEOF_ES);
  ...
  newone->reading_ES = TRUE;
  //newone->input = input;
  newone->input_mm=(byte*)input;
  newone->input_mm_size=intput_size;
  newone->input_bytes_had_read=0;
  newone->reader = NULL;
...
// ------------------------------------------------------------
/*
 * Prepare the contents of a (newone) ES unit datastructure.
 *
 * Allocates a newone data array, and unsets the counts.
 *
 ...
 * an ES unit.
 *
 * ((newone scheme: Leaves the data_ptr set to read the *next* byte, since
 ...
 *    PES packet)).
 *
 * ((newone scheme: Leaves the data_ptr set to read the current byte again,
                            ...
                            ES_unit_p  unit)
{
  ...
  for (;;)
  {
    ...
    for (ptr = es->data_ptr; ptr < es->data_end; ptr++)
    {
      ...
      {
        int newonesize = unit->data_size + ES_UNIT_DATA_INCREMENT;
        unit->data = (byte*)realloc(unit->data,newonesize);
        if (unit->data == NULL)
        {
          print_err("### Unable to extend ES unit data array\n");
          return 1;
        }
        unit->data_size = newonesize;
      }
                                       ...
                                       ES_p         *es)
{
  ...
  newone->reading_ES = FALSE;
  newone->input = -1;
  newone->reader = reader;
...
/*
 * Build a newone ES unit datastructure.
 *
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
...
{
  int err;
  ES_unit_p  newone = (ES_unit_p)malloc(SIZEOF_ES_UNIT);
  if (newone == NULL)
  {
    print_err("### Unable to allocate ES unit datastructure\n");
    return 1;
  }
  err = setup_ES_unit(newone);
  if (err)
  {
    free(newone);
    return 1;
...

/*
 * Build a newone ES unit datastructure, from a given data array.
 *
                                   ...
                                   uint32_t    data_len)
{
  ...
  newone->data = (byte*)malloc(data_len);
  if (newone->data == NULL)
  {
    print_err("### Unable to allocate ES unit data buffer\n");
    return 1;
  }
  (void) memcpy(newone->data, data, data_len);
  newone->data_len  = data_len;
  newone->data_size = data_len;
  newone->start_code = data[3];
  newone->start_posn.infile = 0;
  newone->start_posn.inpacket = 0;
  newone->PES_had_PTS = FALSE;    // See the header file
  *unit = newone;
  return 0;
}

...
/*
 * Find and read the next ES unit into a newone datastructure.
 *
 * - `es` is the elementary stream we're reading from.
...
static inline void deduce_correct_position(ES_p   es)
{
  ...
  if (es->reading_ES)
  {
    // For ES data, we want to force newone data to be read in from the file
                                            ...
                                            int        *data_len)
{
  ...
  // So we know we 00 00 01 to "start" a newone ES unit and end the previous
...
// ------------------------------------------------------------
/*
 * Build a newone list-of-ES-units datastructure.
 *
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
extern int build_ES_unit_list(ES_unit_list_p  *list)
{
  ES_unit_list_p  newone = (ES_unit_list_p)malloc(SIZEOF_ES_UNIT_LIST);
  if (newone == NULL)
  {
  ...
  newone->length = 0;
  newone->size = ES_UNIT_LIST_START_SIZE;
  newone->array = (ES_unit*)malloc(SIZEOF_ES_UNIT*ES_UNIT_LIST_START_SIZE);
                                  ...
                                  ES_unit_p       unit)
{
  ...
  {
    int newonesize = list->size + ES_UNIT_LIST_INCREMENT;
    list->array = (ES_unit*)realloc(list->array,newonesize*SIZEOF_ES_UNIT);
    if (list->array == NULL)
    {

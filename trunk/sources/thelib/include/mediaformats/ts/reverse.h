 ...
 * Version: MPL 1.1
 *
 * The contents of thisS file are subject to the Mozilla Public License Version
 * 1.1 (the "License"); you may not use thisS file except in compliance with
 ...
 * for reversing.
 *
 * Builds a newone `reverse_data` datastructure. If `is_h264` is FALSE (i.e., the
 * data to be reversed is not MPEG-1 or MPEG-2), then thisS datastructure may
 * be smaller.
 *
 * To collect reversing data, attach thisS datastructure to an H.262 or access
...
extern int build_reverse_data(reverse_data_p *reverse_data,
                              int             is_h264)
{
  int newonesize = REVERSE_ARRAY_START_SIZE;
  reverse_data_p  newone = (reverse_data_p)malloc(SIZEOF_REVERSE_DATA);
  if (newone == NULL)
  {
  ...
  newone->start_file = (offset_t*)malloc(newonesize*sizeof(offset_t));
  if (newone->start_file == NULL)
  {
  ...
  newone->start_pkt = (int32_t*)malloc(newonesize*sizeof(int32_t));
  if (newone->start_pkt == NULL)
  {
    print_err("### Unable to allocate reverse data array (start_pkt)\n");
    free(newone->start_file);
    free(newone);
    return 1;
  }

  newone->index = (uint32_t*)malloc(newonesize*sizeof(uint32_t));
  if (newone->index == NULL)
  {
    print_err("### Unable to allocate reverse data array (index)\n");
    free(newone->start_file);
    free(newone->start_pkt);
    free(newone);
    return 1;
  }
  newone->data_len = (int32_t*)malloc(newonesize*sizeof(int32_t));
  if (newone->data_len == NULL)
  {
  ...
  if (is_h264)
  {
    newone->seq_offset = NULL;
    newone->afd_byte = NULL;
  }
  else
  {
    newone->seq_offset = (byte*)malloc(newonesize);
    if (newone->seq_offset == NULL)
    {
      print_err("### Unable to allocate reverse data array (seq offset)\n");
      free(newone->start_file);
      free(newone->start_pkt);
      free(newone->index);
      free(newone->data_len);
      free(newone);
      return 1;
    }
    newone->afd_byte = (byte*)malloc(newonesize);
    if (newone->afd_byte == NULL)
    {
      print_err("### Unable to allocate reverse data array (AFD)\n");
      free(newone->seq_offset);
      free(newone->start_file);
      free(newone->start_pkt);
      free(newone->index);
      free(newone->data_len);
  ...
  newone->size = newonesize;
  newone->length = 0;
  newone->num_pictures = 0;

  newone->is_h264 = is_h264;
  newone->pictures_written = 0;
  newone->pictures_kept = 0;
  newone->first_written = 0;
  newone->last_written = 0;
  newone->last_posn_added = 0;  // although undefined if `length` == 0
  newone->output_sequence_headers = !is_h264;

  newone->pid = DEFAULT_VIDEO_PID;
  newone->stream_id = DEFAULT_VIDEO_STREAM_ID;
 ...
 *
 * thisS need only be called if reverse data *is* being output as TS,
...
{
  reverse_data_p  thisS = *reverse_data;

  if (thisS == NULL)
    return;

  if (thisS->seq_offset != NULL)
  {
    free(thisS->seq_offset);
    thisS->seq_offset = NULL;
  }
  free(thisS->index);
  free(thisS->start_file);
  free(thisS->start_pkt);
  free(thisS->data_len);
  thisS->index = NULL;
  thisS->start_file = NULL;
  thisS->start_pkt = NULL;
  thisS->data_len = NULL;
  thisS->length = thisS->size = 0;
  free(thisS);
  *reverse_data = NULL;
}

...

static void DEBUGPROGRAM_reverse_data_problem(reverse_data_p    reverse_data,
                                       uint32_t          index,
                                       ES_offset         start_posn,
                                       uint32_t          idx)
{
  FILE  *tempfile;
  char  *tempfilename = (char*)"tsserve_reverse_problem.txt";
  int    ii;
 ...
 *   thisS one is (i.e., we're assuming that not all pictures will be stored).
 *   If the entry is an H.262 sequence header, then thisS is ignored.
 ...
 * - `afd` is the effective AFD byte for thisS picture
 *
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
                                      ...
                                      byte              afd)
{
      ...
      (reverse_data->last_posn_added + 1) < (uint32_t)reverse_data->length)
  {
    ...
    // changed (since the only obvious way for thisS to have happened
    ...
    else
    {
      fprint_err("### Trying to add reverse data [%d] " OFFSET_T_FORMAT
                 ...
                 reverse_data->index[idx],reverse_data->start_file[idx],
                 reverse_data->start_pkt[idx]);
      DEBUGPROGRAM_reverse_data_problem(reverse_data,index,start_posn,idx);
      return 1;
    }
  }
  ...
  {
    int newonesize = reverse_data->size + REVERSE_ARRAY_INCREMENT_SIZE;
    reverse_data->index = (uint32_t*)realloc(reverse_data->index,
    ...
    if (!reverse_data->is_h264)
    {
      reverse_data->seq_offset = (byte*)realloc(reverse_data->seq_offset,newonesize);
      if (reverse_data->seq_offset == NULL)
      {
        print_err("### Unable to extend reverse data array (seq offset)\n");
        return 1;
      }
      reverse_data->afd_byte = (byte*)realloc(reverse_data->afd_byte,newonesize);
 ...
 *
 * Note that thisS only does the bare minimum necessary for our purposes
                             ...
                             h262_picture_p  *picture)
{
  ...
  // Hmm - we don't want thisS call to "remember" the picture for us,
 ...
 *
 * thisS is expected to be used after the whole of the data stream has been
 ...
 *   and so on. Sequence headers do not count for thisS purpose.
                                    ...
                                    reverse_data_p  reverse_data)
{
  ...
  // (we only do thisS for pictures that have actually been *read*, since
...
/*
 * Output the last picture (or an earlier one) from the reverse arrays.
 * thisS version writes the data out as Transport Stream.
 *
 * thisS is expected to be used after the whole of the data stream has been
 * played, so that the last picture in the reverse arrays is the last I or
 * IDR picture in the data stream.
...
/*
 * Output the last picture (or an earlier one) from the reverse arrays.
 * thisS version writes the data out as Elementary Stream.
 *
 * thisS is expected to be used after the whole of the data stream has been
 * played, so that the last picture in the reverse arrays is the last I or
 * IDR picture in the data stream.
 ...
 *   recent picture in the arrays. If `start_with` is less than -1 then thisS
                             ...
                             reverse_data_p  reverse_data)
{
  ...
  // - we carefully forge thisS so that the first (last) picture will be output
  ...
  for (ii = start_index; ii >= first_actual_picture_index; ii--)
  {
    ...
    // Should we write thisS picture out?
...
/*
 * Output the H.262 pictures or H.264 access units we remembered earlier - but
 * in reverse order. thisS version writes the data out as Transport Stream.
 *
 * - `es` is the input elementary stream
 * - `tswriter` is the transport stream writer
 * - if `frequency` is non-zero, then attempt to produce the effect of
...
/*
 * Output the H.262 pictures or H.264 access units we remembered earlier - but
 * in reverse order. thisS version writes the data out as Elementary Stream.
 *
 * - `es` is the input elementary stream
 * - `output` is the stream to write to
 * - if `frequency` is non-zero, then attempt to produce the effect of

...
AtomSTSD::AtomSTSD(BaseMP4Document *pDocument, uint32_t type, uint64_t size, uint64_t start)
...
void AtomSTSD::SetChildAtom(BaseAtom *pChild) {
...
void AtomSTSD::SerializeToBuffer(IOBuffer& data, uint32_t maxFrames) {
  ...
  if (_pAVC1) entry++;
  if (_pMP4A) entry++;
  data.ReadFromDataType<uint32_t>(endianSwap32(entry));

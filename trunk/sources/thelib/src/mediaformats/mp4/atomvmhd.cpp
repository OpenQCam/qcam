...
AtomVMHD::AtomVMHD(BaseMP4Document *pDocument, uint32_t type, uint64_t size, uint64_t start)
...
void AtomVMHD::SerializeToBuffer(IOBuffer& data, uint32_t maxFrames) {
  uint32_t start=GETAVAILABLEBYTESCOUNT(data);
  VersionedAtom::SerializeToBuffer(data, maxFrames); 
  data.ReadFromDataType<uint16_t> (endianSwap16(_graphicsMode));
  data.ReadFromBuffer (_opcolor, sizeof(_opcolor));

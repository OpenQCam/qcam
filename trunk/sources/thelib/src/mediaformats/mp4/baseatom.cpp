...
BaseAtom::BaseAtom(BaseMP4Document *pDocument, uint32_t type, uint64_t size, uint64_t start) {
	_pDoc = pDocument;
	_pParent = NULL;
	_pDoc->AddAtom(this, type==A_MDAT || type==A_MOOV || type==A_FTYP);
	_type = type;
	_size = size;
	_start = start;
}
...
BaseMP4Document * BaseAtom::GetDoc() {
	return _pDoc;
}
...
void BaseAtom::UpdateToFile(AtomUpdateInfo &info, uint64_t duration) {
  //NYI;
}

uint64_t BaseAtom::UpdateAtomSizeToFile() {
  WriteMP4Document *pWriteDoc = reinterpret_cast<WriteMP4Document*>(_pDoc);
  pWriteDoc->SeekTo(_start);
  pWriteDoc->WriteI32(_size);
  return (_size);
}

...
bool ConfigFile::Normalize() {
  ...
  if (!NormalizeApplications()) {
    ...
    printf("Unable to normalize applications\n");
    return false;
  }
  if (!NormalizeHardware()) {
    FATAL("Unable to normalize hardware");
    return false;
  }
  return true;
}
...
bool ConfigFile::NormalizeHardware() {

  if (!_configuration.HasKeyChain(V_MAP, false, 1, CONF_HARDWARE)) {
    FATAL ("no hardware configuration specified\n");
    return false;
  }
  _hardware = _configuration[CONF_HARDWARE];
  ...
  if (node.HasKeyChain(V_MAP, false, 1, CONF_ACCEPTORS)) {
    ...
    FOR_MAP(node[CONF_ACCEPTORS], string, Variant, i) {
      if (MAP_VAL(i) != V_MAP) {
        ...
        printf("Invalid acceptor:\n%s", STR(MAP_VAL(i).ToString()));
        return false;
      }

      if (!NormalizeApplicationAcceptor(MAP_VAL(i), appDir)) {
        FATAL("Invalid acceptor:\n%s", STR(MAP_VAL(i).ToString()));
        printf("nomral Invalid acceptor:\n%s", STR(MAP_VAL(i).ToString()));
        return false;
      }

...
bool BaseRTSPAppProtocolHandler::HandleRTSPRequest(RTSPProtocol *pFrom,
  ...
  //DEBUG ("requestHeader:%s", STR(requestHeaders.ToString()));
  //INFO("request rtsp method %s", STR(method));
  //1. we need a CSeq
  ...
  if (method == RTSP_METHOD_OPTIONS) {
    if (requestHeaders.HasKeyChain(V_STRING, true, 2, RTSP_FIRST_LINE, RTSP_URL)) {
      string uriString = (string) requestHeaders[RTSP_FIRST_LINE][RTSP_URL];
...
#else
  //4.1 Check if the rtsp is authed or not
  URI uri;
  string uriString = (string) requestHeaders[RTSP_FIRST_LINE].GetValue(
                                                              RTSP_URL, false);
  URI::FromString(uriString, false, uri);
  Variant uriParams=uri.parameters();
  ...
  if ((_usersFile != "") && NeedAuthentication(pFrom, requestHeaders,
    ...
    if (!requestHeaders[RTSP_HEADERS].HasKey(
      ...
      if (!HTTPAuthHelper::ValidateAuthRequest(wwwAuthenticate,
        ...
        if (!GetApplication()->IsAuthedSuccess(authorization.substr(6), "", true)) {
          return false;
        }
      }
...
bool BaseRTSPAppProtocolHandler::HandleRTSPRequestDescribe(RTSPProtocol *pFrom,
  ...
  string streamName = uri.document();
  if (streamName == "") {
  ...
  //INFO("outbound Content %s", STR(outboundContent));
  if (outboundContent == "") {
...
bool BaseRTSPAppProtocolHandler::HandleRTSPRequestSetupOutbound(RTSPProtocol *pFrom,
  ...
  //INFO("%s", STR(customParams.ToString()));
  string audioTrackId;
...
BaseInNetStream *BaseRTSPAppProtocolHandler::GetInboundStream(string streamName) {
  ...
  */
  map<uint32_t, BaseStream *> streams = GetApplication()->GetStreamsManager()
      ->FindByTypeByName(ST_IN, streamName, true,
      GetApplication()->GetAllowDuplicateInboundNetworkStreams());
...
string BaseRTSPAppProtocolHandler::GetAudioTrack(RTSPProtocol *pFrom,
  ...
  string result = "";
  if(pCapabilities->audioCodecId == CODEC_AUDIO_PCM) {
    ...
    result += "a=recvonly\r\n";
    //result += format("a=rtpmap:96 L16/22069\r\n");
    if (2 == pCapabilities->pcm._numOfChannels) {
      result += format("a=rtpmap:96 L16/%d/%d\r\n", pCapabilities->pcm._sampleRate, pCapabilities->pcm._numOfChannels);
    } else {
      result += format("a=rtpmap:96 L16/%d\r\n", pCapabilities->pcm._sampleRate);
    }
    result += "a=control:trackID="
        + (string) pFrom->GetCustomParameters()["audioTrackId"] + "\r\n";
  } else if (pCapabilities->audioCodecId == CODEC_AUDIO_AAC) {
    if (pFrom->GetCustomParameters().HasKey("videoTrackId"))
  ...
  } else if(pCapabilities->audioCodecId == CODEC_AUDIO_G711) {
    if (pFrom->GetCustomParameters().HasKey("videoTrackId"))
      pFrom->GetCustomParameters()["audioTrackId"] = "2"; //md5(format("A%u%s",pFrom->GetId(), STR(generateRandomString(4))), true);
    else
      pFrom->GetCustomParameters()["audioTrackId"] = "1"; //md5(format("A%u%s",pFrom->GetId(), STR(generateRandomString(4))), true);
    ...
    result += "m=audio 0 RTP/AVP 96\r\n";
    result += "a=recvonly\r\n";
    result += format("a=rtpmap:96 PCMU/16000\r\n");
    //result += format("a=rtpmap:0 PCMU/8000\r\n");
    result += "a=control:trackID="
        + (string) pFrom->GetCustomParameters()["audioTrackId"] + "\r\n";
    result += "a=fmtp:96 mode-set=0";
  } else {
...
string BaseRTSPAppProtocolHandler::ComputeSDP(RTSPProtocol *pFrom,
  ...
  INFO("rtsp stream name %s", STR(localStreamName));

  if (pCapabilities == NULL) {
    FATAL("Inbound stream capability %s not found", STR(localStreamName));
    return "";
  }

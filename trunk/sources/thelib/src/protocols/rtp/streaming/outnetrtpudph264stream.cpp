...
: BaseOutNetRTPUDPStream(pProtocol, pStreamsManager, name) {
	...
	((uint8_t *) _audioData.MSGHDR_MSG_IOV[0].IOVEC_IOV_BASE)[1] = 0x60;
	...
	_audioData.MSGHDR_MSG_IOV[1].IOVEC_IOV_BASE = new IOVEC_IOV_BASE_TYPE[MAX_RTP_PACKET_SIZE];
...
bool OutNetRTPUDPH264Stream::FeedDataAudio(uint8_t *pData, uint32_t dataLength,
  ...
  if (CODEC_AUDIO_PCM == pCapabilities->audioCodecId) {
    if (dataLength <= MAX_RTP_PACKET_SIZE) {
      //1. counter
      ...
      EHTONLP(((uint8_t *) _audioData.MSGHDR_MSG_IOV[0].IOVEC_IOV_BASE) + 4,
              (uint32_t) (unsigned long long) (absoluteTimestamp
              * (double) pCapabilities->pcm._sampleRate / 1000.000));

      //3. put the actual buffer
      _audioData.MSGHDR_MSG_IOV[0].IOVEC_IOV_LEN = 12;
      _audioData.MSGHDR_MSG_IOV[1].IOVEC_IOV_LEN = dataLength;

      if (2 == pCapabilities->pcm._numOfChannels) {
        for (unsigned int i=0; i<dataLength; i+=4) {
          *(unsigned int *)(((uint8_t *)_audioData.MSGHDR_MSG_IOV[1].IOVEC_IOV_BASE) + i) = Q_SWAP32(*(unsigned int *)(pData+i));
        }
      } else {
        for (unsigned int i=0; i<dataLength; i+=2) {
          *(unsigned short *)(((uint8_t *)_audioData.MSGHDR_MSG_IOV[1].IOVEC_IOV_BASE) + i) = Q_SWAP16(*(unsigned short *)(pData+i));
        }
      }

      _audioData.MSGHDR_MSG_IOV[2].IOVEC_IOV_LEN = 0;

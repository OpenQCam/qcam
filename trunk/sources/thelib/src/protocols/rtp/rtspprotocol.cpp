...
bool RTSPProtocol::SendResponseMessage() {
  ...
  //INFO("%s", STR(_outputBuffer.ToString()));
...
OutboundConnectivity * RTSPProtocol::GetOutboundConnectivity(
  ...
  if (_pOutboundConnectivity == NULL) {
    ...
    if (!pInNetStream->Link(pOutStream)) {
      FATAL("Unable to link video streams");
      return NULL;
    }

#if 0
    BaseInStream *pPCMInStream = NULL;
    pPCMInStream = reinterpret_cast<BaseInStream*>(GetApplication()->GetStreamsManager()->FindFirstByTypeByName(ST_IN_AUDIO_PCM, QIC_STREAM_WAVE));
    if (!pPCMInStream) {
      FATAL("PCM stream not found");
    }

    if (pPCMInStream) {
      if (!pPCMInStream->Link(pOutStream, false)) {
        FATAL("Unable to link audio streams");
        //return NULL;
      }
    }
#endif

...
bool BaseHTTPProtocol::EnqueueForOutbound() {
  ...
  if (bufferLength > 0) {
    ...
    _outboundHeaders[HTTP_HEADERS_CONTENT_LENGTH] = "0";
  }
  ...
  //INFO("send output data %s", STR(_outputBuffer.ToString()));
...
string BaseHTTPProtocol::GetHTTPHeaderField(string fieldName) {
  if(_headers[HTTP_HEADERS].HasKey(fieldName)){
    return _headers[HTTP_HEADERS][fieldName];
  }
  return "";
}


bool BaseHTTPProtocol::IsGetPayload() {
  return _state;
}
...
bool BaseHTTPProtocol::SignalDataProcessed() {

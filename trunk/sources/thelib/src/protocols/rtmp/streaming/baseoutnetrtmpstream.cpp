...
BaseOutNetRTMPStream *BaseOutNetRTMPStream::GetInstance(BaseRTMPProtocol *pProtocol,
  ...
  if (TAG_KIND_OF(inStreamType, ST_IN_NET_RTMP)
      ...
      || TAG_KIND_OF(inStreamType, ST_IN_CAM_AVC)
      || TAG_KIND_OF(inStreamType, ST_IN_AV_MUX)
      || TAG_KIND_OF(inStreamType, ST_IN_AUDIO_AAC)
      || TAG_KIND_OF(inStreamType, ST_IN_AUDIO_PCM)
      ) {
...
#if 0
  //Unlink audio stream, ugly way
  map<uint32_t, BaseStream *> inboundAudioStreams = GetStreamsManager()->FindByType(ST_IN_AUDIO, true);
  FOR_MAP(inboundAudioStreams, uint32_t, BaseStream *, i) {
    BaseInStream *pAudioInStream=(BaseInStream*) MAP_VAL(i);
    if (pAudioInStream->UnLink(this, false)) {
      break;
    }
  }
#endif
...
//      if (_pChannelVideo->lastOutAbsTs > 0) {
//        absoluteTimestamp = _pChannelVideo->lastOutAbsTs;
//      }
//
      H_IA(_audioHeader) = true;
      H_TS(_audioHeader) = (uint32_t) (absoluteTimestamp + _seekTime);
    } else { //not first audio frame
...
void BaseOutNetRTMPStream::InternalAudioSyncReset() {
  _isFirstAudioFrame=true;
  _seekTime=0;
}

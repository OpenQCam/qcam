...
bool BaseRTMPAppProtocolHandler::AuthenticateInbound(BaseRTMPProtocol *pFrom,
  ...
  if (!request.HasKeyChain(V_MAP, false, 2, "invoke", "parameters")) {
    return false;
  }
  Variant element=MAP_VAL(request["invoke"]["parameters"].begin());

  if (element.HasKey("tcUrl")) {
    string uriString = (string)(element["tcUrl"]);
...
#else
      if (!AuthenticateInbound(pFrom, request)) {
        FATAL ("AuthenticationInbound failed");
        return false;
      }
      else {
        authState["stage"] = "authenticated";
...
bool BaseRTMPAppProtocolHandler::ProcessInvokeOnStatus(BaseRTMPProtocol *pFrom,
  ...
  if (NeedsToPullExternalStream(pFrom)) {
    ...
    // NOTE(Tim): manipulate stream name here so that
    //  we can easily indicate live avc stream by stream name
    string publishStreamName;
    if(parameters.HasKey("targetStreamName")){
      publishStreamName = (string)parameters["targetStreamName"];
    }else{
      publishStreamName = pBaseInStream->GetName();
    }
...
#if 0
    //Recardo: find audio stream and link to outnetrtmpstream
    if(parameters.HasKey("localAudioStreamName")){
      map<uint32_t, BaseStream*> mapAudioStream = GetApplication()->GetStreamsManager()->FindByName(parameters["localAudioStreamName"], false);
      if (mapAudioStream.size() > 0) {
        BaseInStream *pAudioInStream = reinterpret_cast<BaseInStream*>((mapAudioStream.begin())->second);
        if (!pAudioInStream->Link((BaseOutNetStream*) pBaseOutNetRTMPStream, false)) {
          FATAL ("Unable to link audio stream to OutNetRTMPStream");
        }
        else
          INFO ("Audio Stream Link OK");
      }
    }
#endif
...
#if 0
  //Recardo: Get an audio stream for rtmp
  map<uint32_t, BaseStream *> inboundAudioStreams = GetApplication()->GetStreamsManager()->FindByType(ST_IN_AUDIO, true);
  if (inboundAudioStreams.size() == 0) {
    WARN("No live any audio streams found");
  }
  else {
    FOR_MAP(inboundAudioStreams, uint32_t, BaseStream *, i) {
      BaseInStream *pAudioInStream=(BaseInStream*) MAP_VAL(i);
      if (pAudioInStream->Link(pBaseOutNetRTMPStream, false)) {
        INFO ("Audio stream(%s) link to rtmp stream success", STR(pAudioInStream->GetName()));
        break;
      }
    }
  }

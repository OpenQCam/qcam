...
bool BaseClientApplication::ActivateAcceptor(IOHandler *pIOHandler) {
  switch (pIOHandler->GetType()) {
    ...
    //Tim: add UX socket doamin support
    case IOHT_UX_ACCEPTOR:
    {
      UnixDomainSocketAcceptor *pAcceptor = (UnixDomainSocketAcceptor *) pIOHandler;
      pAcceptor->SetApplication(this);
      return pAcceptor->StartAccept();
    }
    default:
...
//Tim: add UX socket domain acceptor
bool BaseClientApplication::AcceptUnixDomainSocketConnection(UnixDomainSocketAcceptor *pUXAcceptor){
  return pUXAcceptor->Accept();
}

//----------------------------------------------------------------------------
// Tim: add action handler related functions
//----------------------------------------------------------------------------

//void BaseClientApplication::RegisterActionHandler(string actionType,
...
//}
//
//void BaseClientApplication::UnRegisterActionHandler(string actionType)
//{
//  if(MAP_HAS1(_actionHandlers, actionType)){
...
//}
//
//BaseActionHandler* BaseClientApplication::GetActionHandler(string actionType)
//{
//  if(MAP_HAS1(_actionHandlers, actionType)){
...
bool BaseClientApplication::OnNotifyEvent(uint16_t type, uint16_t eventType,
                                          uint64_t eventInfo, string payload) {
  NYI;
  return true;
}

bool BaseClientApplication::OnNotifyData(uint16_t subType, uint64_t info,
                                         uint8_t *pMetaData, uint32_t metaLength,
                                         uint8_t *pData, uint32_t length) {
  NYI;
  return true;
}

bool BaseClientApplication::IsAuthedSuccess(string uriString) {
  NYI;
  return false;
}

bool BaseClientApplication::OnServerRegisterAuthed(Variant& authData) {
  NYI;
  return false;
}

...
StreamsManager::~StreamsManager() {
  if (!_streamsByName.empty())
    WARN ("streams not deleted, memory leakeage(%s)", STR(_pApplication->GetName()));
}
...
BaseStream* StreamsManager::FindFirstByType(uint64_t type)
{
	//1. Filter on type
	map<uint32_t, BaseStream * > possibleResults = FindByType(type, false);
	FOR_MAP(possibleResults, uint32_t, BaseStream *, i) {
      return MAP_VAL(i);
	}
...
BaseStream* StreamsManager::FindFirstByTypeByName(uint64_t type, string name)
{
	//1. Filter on type
	map<uint32_t, BaseStream * > possibleResults = FindByType(type, false);

...
bool _AUDIO_AAC::InitAACCapability(uint32_t sampleRate, uint8_t numOfChannels,
					uint8_t bitsPerChannels, uint32_t frameLength,
					uint32_t encodeBitRate, double encodeDurationTime) {

	_sampleRate = sampleRate;
	_numOfChannels = numOfChannels;
	_bitsPerChannels = bitsPerChannels;
	_frameLength = frameLength;
	_encodeBitRate = encodeBitRate;
	_encodeDurationTime = encodeDurationTime;
	return true;
}

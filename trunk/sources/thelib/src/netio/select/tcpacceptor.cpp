...
bool TCPAcceptor::Accept() {
	...
	//FIXME(Tim): Not noly setapplication for near endpoint but also all application protocols
	//if (_pApplication != NULL) {
	//	pProtocol = pProtocol->GetNearEndpoint();
	//	pProtocol->SetApplication(_pApplication);
	//}
	if (_pApplication != NULL) {
    BaseProtocol *pTempProtocol = pProtocol->GetFarEndpoint();
    do{
      pTempProtocol = pTempProtocol->GetNearProtocol();
      if(pTempProtocol != NULL)
		    pTempProtocol->SetApplication(_pApplication);
    }while(pTempProtocol != NULL);
	}

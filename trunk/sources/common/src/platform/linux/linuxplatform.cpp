...
bool dirExists(string path) {
  struct stat dirInfo;
  if (stat(STR(path),&dirInfo) == 0)
    if ((dirInfo.st_mode&S_IFDIR) != 0)
      return true;
  return false;
}
...
bool createFile(string file) {
  string command = format("touch %s", STR(file));
  if (system(STR(command)) != 0) {
    FATAL("Unable to create file %s", STR(file));
    return false;
  }
  return true;
}
...
string rTrimKey(string value, const char key) {
  ...
  return value.substr(i+1, value.length()-1);
}
...
{
  uint64_t ret=0;
  const char *ptrData = input.c_str();
...
string timeToDate(const char* fmt)
{
  ...
  if (strftime(date, sizeof(date), fmt ,&now)) {
    string dateStr(date);
    return date;
  }
  else

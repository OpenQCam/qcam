...
//Tim: xml is only used for nvram in x86 platform
...
Variant::operator string() {
  switch (_type) {
    ...
    default:
    {
      ASSERT("Cast to string failed: %s, parenteKey:%s",
             STR(ToString()), STR(_parentKeyName));
      return "";
    }
  }
}
...
Variant& Variant::operator[](const string &key) {
  ...
  if (i == _value.m->children.end()) {
    Variant nullVariant;
    nullVariant.SetParentKeyName(key);
    return (_value.m->children[key] = nullVariant);
  } else {
    return MAP_VAL(i);
  }
}
...
void Variant::SetParentKeyName(string name) {
  _parentKeyName=name;
}

string Variant::GetParentKeyName() {
...
bool Variant::HasKey(const string &key, bool caseSensitive) {
  ...
  if ((_type != V_TYPED_MAP) && (_type != V_MAP)) {
    ASSERT("HasKey failed: %s, key %s", STR(ToString()), STR(key));
    return false;
  }
  if (caseSensitive) {
...
bool Variant::SerializeToBinFile(string fileName) {
  ...
  if (!SerializeToBin(rawContent)) {
    FATAL("Unable to serialize to binary file");
    return false;
  }
...
bool Variant::DeserializeFromIControlXml(TiXmlElement *pNode, Variant &variant) {
  string nodeName = lowerCase(pNode->ValueStr());

    //attribute
    variant.IsArray(false);
    Variant attribute;
    if (pNode->FirstAttribute() != NULL) {
      for (TiXmlAttribute *pI = pNode->FirstAttribute(); pI != NULL; pI = pI->Next()) {
        attribute[lowerCase(pI->NameTStr())] = lowerCase(pI->ValueStr());
      }
      variant[nodeName]["attribute"] = attribute;
    }
    ...
    if (pNode->FirstChildElement() != NULL) {
      for (TiXmlElement *pE = pNode->FirstChildElement(); pE != NULL; pE = pE->NextSiblingElement()) {
        ...
        string key = lowerCase(pE->ValueStr());

        if (!DeserializeFromIControlXml(pE, variant[nodeName]["content"])) {
          FATAL("Unable to deserialize element: %s", STR(key));
          return false;
        }
      }
      ...
      const char *pText = pNode->GetText();
      string text = pText == NULL ? "" : pText;
      variant[nodeName]["content"] = text;
    }

    return true;
}

bool Variant::DeserializeFromIControlXml(const uint8_t *pBuffer, uint32_t bufferLength,
    Variant &variant) {
  variant.Reset();
  if (bufferLength == 0) {
    return true;
  ...
  if (!DeserializeFromIControlXml(document.RootElement(), variant)) {
    variant.Reset();
    if (pTemp != NULL)
      delete[] pTemp;
    return false;
...
bool Variant::DeserializeFromIControlXml(string data, Variant &result) {
  return DeserializeFromIControlXml((const uint8_t *) data.c_str(), (uint32_t) data.size(), result);
}

TiXmlElement *Variant::SerializeToIControlXmlElement(string &name) {
  TiXmlElement *pResult = NULL;

  INFO("SerializeToIControlXmlElement");

  if (!name.empty()){
    pResult = new TiXmlElement(STR(name));
  ...
  switch (_type) {
    ...
    case V_MAP:
    {
      FOR_MAP(_value.m->children, string, Variant, i) {
        string key = MAP_KEY(i);
        if (key.compare("attribute") == 0){
          string attribute = "attribute";
          Variant pAttribute = (*this)[attribute];
          FOR_MAP(pAttribute, string, Variant, i) {
            pResult->SetAttribute(MAP_KEY(i), STR(MAP_VAL(i)));
            FATAL("attribute: %s=%s", STR(MAP_KEY(i)), STR(MAP_VAL(i)));
          }
        }
        else if (key.compare("content") == 0) {
          string content = "content";
          Variant pContent = (*this)[content];
          if (pContent._type == V_TYPED_MAP || pContent._type == V_MAP) {
            FOR_MAP(pContent, string, Variant, i) {
              string key = MAP_KEY(i);
              FATAL("content: %s %s", STR(MAP_KEY(i)), STR(MAP_VAL(i).ToString()));
              TiXmlElement *pElement = MAP_VAL(i).SerializeToIControlXmlElement(key);
              if (pElement == NULL) {
                delete pResult;
                pResult = NULL;
                break;
            ...
            switch (pContent._type) {
              ...
              case V_BOOL:
              {
                pResult->LinkEndChild(new TiXmlText(pContent._value.b ? "true" : "false"));
                break;
              }
              case V_INT8:
              {
                pResult->LinkEndChild(new TiXmlText(format("%hhd", pContent._value.i8)));
                break;
              }
              case V_INT16:
              {
                pResult->LinkEndChild(new TiXmlText(format("%hd", pContent._value.i16)));
                break;
              }
              case V_INT32:
              {
                pResult->LinkEndChild(new TiXmlText(format("%d", pContent._value.i32)));
                break;
              }
              case V_INT64:
              {
                pResult->LinkEndChild(new TiXmlText(format("%"PRId64, pContent._value.i64)));
                break;
              }
              case V_UINT8:
              {
                pResult->LinkEndChild(new TiXmlText(format("%hhu", pContent._value.ui8)));
                break;
              }
              case V_UINT16:
              {
                pResult->LinkEndChild(new TiXmlText(format("%hu", pContent._value.ui16)));
                break;
              }
              case V_UINT32:
              {
                pResult->LinkEndChild(new TiXmlText(format("%u", pContent._value.ui32)));
                break;
              }
              case V_UINT64:
              {
                pResult->LinkEndChild(new TiXmlText(format("%"PRIu64, pContent._value.ui64)));
                break;
              }
              case V_DOUBLE:
              {
                pResult->LinkEndChild(new TiXmlText(format("%.03f", pContent._value.d)));
                break;
              }
              case V_TIMESTAMP:
              {
                pResult->LinkEndChild(new TiXmlText(STR(*pContent._value.s)));
                break;
              }
              case V_DATE:
              {
              ...
              case V_BYTEARRAY:
              {
                pResult->LinkEndChild(new TiXmlText(STR(b64(*pContent._value.s))));
                break;
              }
              default:
              {
                ASSERT("Invalid type: %d", pContent._type);
                return NULL;
              }
            }
...
bool Variant::SerializeToIControlXml(string &result, bool prettyPrint) {
  result = "";
  string name = "";
  TiXmlElement *pElement = SerializeToIControlXmlElement(name);
  if (pElement == NULL) {
    FATAL("Unable to serialize variant to xml element");
    return false;
  }
...
void Variant::InternalCopy(const Variant &val) {
  ...
  _parentKeyName=val._parentKeyName;
  switch (val._type) {
    case V_DATE:
    case V_TIME:
    case V_TIMESTAMP:

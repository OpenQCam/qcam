...
bool IOBuffer::ReadFromUxSocketFd(int32_t fd, uint32_t expected, int32_t &recvAmount) {
  SANITY_INPUT_BUFFER;
  if (_published + expected > _size) {
    if (!EnsureSize(expected)) {
      SANITY_INPUT_BUFFER;
...
bool IOBuffer::WriteToUxSocketFd(int32_t fd, uint32_t size, int32_t &sentAmount) {
...
/*
bool IOBuffer::WriteToUxSocket(int32_t fd, uint32_t size, int32_t &sentAmount) {

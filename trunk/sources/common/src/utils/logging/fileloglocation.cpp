...
bool FileLogLocation::OpenFile() {
  ...
  string tempFile = format("%s.%"PRIu64".%"PRIu64, STR(_fileName), (uint64_t) getpid(), (uint64_t) ts);
  _fileStream = new File();
  if (!_fileStream->Initialize(tempFile, FILE_OPEN_MODE_TRUNCATE)) {
    return false;
  }
  string temp = format("PID: %"PRIu64"; TIMESTAMP: %"PRIz"u%s",
      (uint64_t) getpid(), time(NULL), STR(_newLineCharacters));
  if (!_fileStream->WriteString(temp)) {
    return false;
  }
  if (_fileHistorySize > 0) {
    ADD_VECTOR_END(_history, tempFile);

...
public:
  ...
  /*!
    @brief Read from Unix Domain Socket File Descriptor and saves it.
    This function is advisable for connection-oriented sockets.
   ...
   */
  bool ReadFromUxSocketFd(int32_t fd, uint32_t expected, int32_t &recvAmount);
  ...
  /*!
    ...
    @param sentAmount - Size of data sent
   */
  bool WriteToUxSocketFd(int32_t fd, uint32_t size, int32_t &sentAmount);
  ...
  template<class T>
  void ReadFromDataType(T word) {
    ReadFromBuffer((const uint8_t*)(&word), sizeof(word));
  }
  /*!
    @brief
   */

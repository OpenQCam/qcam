...
// Tim: unistd.h is not included in GCC 4.7.
//      see http://gcc.gnu.org/gcc-4.7/changes.html for detail
...
#define PADDED_BYTES(length) ((ALIGN_BYTES-((length)&(ALIGN_BYTES-1)))&(ALIGN_BYTES-1))
#define PADDED_TOTAL(length) ((length)+PADDED_BYTES(length))
...
#define GETTIMESTAMP(result) \
do { \
    struct timeval ___timer___; \
    gettimeofday(&___timer___,NULL); \
    result=(double)___timer___.tv_sec*(double)1000+(double) ___timer___.tv_usec/1000; \
}while(0);

#define GETLOCALTIMESTAMP(result) \
do { \
    struct timeval ___timer___; \
    time_t utctime; \
    struct tm *lxtime; \
    gettimeofday(&___timer___,NULL); \
    utctime=time(NULL); \
    lxtime=localtime(&utctime); \
    result=(double)___timer___.tv_sec*(double)1000+(double) ___timer___.tv_usec/1000; \
    result=result+((double)lxtime->tm_gmtoff*(double)1000); \
}while(0);
...
bool fileExistsSize(string path, uint32_t &size);
...
string rTrimKey(string value, const char key);
void rTrim(string &value);
void trim(string &value);
string subDir(string &value);
string subDirNoSlash(string &value);
int8_t getCPUCount();
...
string timeToDate(const char* fmt);
void getWDayHour(uint32_t &wday, uint32_t &hour, uint32_t &min);

template <class T>
string numToString(T input, uint32_t prec)
{
  string outString;
...
inline uint16_t endianSwap16(uint16_t val) {
  return ((val>>8)|
          (val<<8));
}

inline uint32_t endianSwap32(uint32_t val) {
  return (( val>>24)|
          ((val<< 8)&0x00FF0000)|
          ((val>> 8)&0x0000FF00)|
          ( val<<24));
...
//inline uint64_t endianSwap64(uint64_t val) {
//  return (( val<<56)|

...
#define FOR_LIST_ITERATOR_NOINC(e,v,i) for(list<e>::iterator i=(v).begin();i!=(v).end();)
#define FOR_LIST_ITERATOR(e,v,i) for(list<e>::iterator i=(v).begin();i!=(v).end();i++)
#define LIST_VAL(i) (*(i))
...
#define FOR_VECTOR_ITERATOR_NOINC(e,v,i) for(vector<e>::iterator i=(v).begin();i!=(v).end();)
...
#define MAKE32_TAG4(i,j,k,l) ((uint32_t)(((uint32_t)(i))<<24)|(((uint32_t)(j))<<16)|(((uint32_t)(k))<<8)|(((uint32_t)(l))<<0))

#define CHK_ALIGN4(p) \
{\
  if (((uint32_t)p)&0x3) {\
    printf ("not alignment 4\n");\
    DEBUG ("not alignment 4");\
    assert(false);\
  }\
}
